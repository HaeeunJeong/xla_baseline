...
# -*- coding: utf-8 -*-
"""
pytorch_export.py – PyTorch torch.export mode latency & memory probe.
"""
...
def time_forward(model: torch.nn.Module,
    ...
    * measured repetitions: `repeats`, averaged
    ...
    if isinstance(dummy_input, torch.Tensor):
        ...
        executable = ep.module()
        call = lambda: executable(inp)
        ...
        if all(isinstance(t, torch.Tensor) for t in dummy_input):
            ...
            ep = torch.export.export(model, tuple(tensors))
            executable = ep.module()
            call = lambda: executable(*tensors)
        # ➋ tuple of ints: shape 지정 → 랜덤 Tensor 생성
        else:
            x = torch.randn(*dummy_input, device=device_obj)
            ep = torch.export.export(model, (x,))
            executable = ep.module()
            call = lambda: executable(x)
...
def main():
    ...
    default_path = RESULT_DIR / f"metrics_export_{tag}_{actual_device}.csv"
