...
# -*- coding: utf-8 -*-
"""
pytorch_lazy.py – Universal PyTorch latency & memory probe with JIT weight loading (Lazy offloading).
"""
...
def time_forward(model: torch.nn.Module,
    ...
    * measured repetitions: `repeats`, averaged
    ...
    model = model.eval()  # Keep root model structure on CPU initially

    if device_obj.type == "cuda":
        # --------------------- Lazy Loading Hooks 부착 ----------------------
        def _pre_hook(m, inputs):
            torch.cuda.nvtx.range_push(f"HtoD_{m.__class__.__name__}")
            for p in m.parameters(recurse=False):
                if p is not None:
                    p.data = p.data.to(device_obj)
            for b in m.buffers(recurse=False):
                if b is not None:
                    b.data = b.data.to(device_obj)
        ...
        # 모든 모듈에 대해 직접 보유한 파라미터나 버퍼가 있다면 Hook 부착
        for module in model.modules():
            has_direct_params = any(True for _ in module.parameters(recurse=False))
            has_direct_buffers = any(True for _ in module.buffers(recurse=False))
